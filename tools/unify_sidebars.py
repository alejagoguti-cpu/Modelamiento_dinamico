...
def make_sidebar(active_file):
    ...
    <a class="brand-icon" href="dashboard.html" title="Bogotá Viva · Dashboard" aria-label="Dashboard"><i class="fa-solid fa-diagram-project"></i></a>
...
for number in ('03', '04', '06', '07', '08'):
    path = root / f'modulo-{number}.html'
    text = path.read_text(encoding='utf-8')
    pattern = r'  <aside class="sidebar sidebar-icons-only">.*?</aside>'
    replacement = make_sidebar(f'modulo-{number}.html')
    updated, count = re.subn(pattern, replacement, text, count=1, flags=re.S)
    if count != 1:
        raise SystemExit(f'No se pudo localizar exactamente una sidebar en {path}')
