...
(() => {
  ...
  let dragging = false, lastPointer = null, penMode = false, penPoints = [];
  let drawnRings = JSON.parse(localStorage.getItem('sumoModule8DrawnWetlands') || '[]');
  ...
  function draw() {
    ...
    ctx.strokeStyle = '#ffd166'; ctx.fillStyle = 'rgba(255,209,102,.22)'; ctx.lineWidth = 2.5; ctx.setLineDash([7, 5]);
    for (const ring of [...drawnRings, penPoints]) if (ring.length) { ctx.beginPath(); const a = toScreen(ring[0][0], ring[0][1]); ctx.moveTo(a[0], a[1]); for (let i = 1; i < ring.length; i++) { const p = toScreen(ring[i][0], ring[i][1]); ctx.lineTo(p[0], p[1]); } if (ring.length > 2 && ring !== penPoints) { ctx.closePath(); ctx.fill(); } ctx.stroke(); }
    ctx.setLineDash([]); ctx.restore();
  }
  function worldPoint(event) { const r = dragSurface.getBoundingClientRect(); return [(event.clientX - r.left - view.offX) / view.scale, -((event.clientY - r.top - view.offY) / view.scale)]; }
  function finishPen() { if (penPoints.length < 3) return; drawnRings.push(penPoints.slice()); penPoints = []; localStorage.setItem('sumoModule8DrawnWetlands', JSON.stringify(drawnRings)); renderPenOutput(); draw(); }
  function renderPenOutput() { const out = document.getElementById('sumoPenOutput'); if (out) out.value = JSON.stringify({ format: 'SUMO-local-coordinates', polygons: drawnRings.map(r => r.map(([x, y]) => ({ x: Number(x.toFixed(2)), y: Number(y.toFixed(2)) }))) }, null, 2); }
  ...
  dragSurface.addEventListener('dblclick', event => { if (penMode) { event.preventDefault(); finishPen(); } });
  dragSurface.addEventListener('pointerdown', event => { if (!visible) return; event.preventDefault(); if (penMode) { penPoints.push(worldPoint(event)); draw(); return; } dragging = true; lastPointer = { x: event.clientX, y: event.clientY }; dragSurface.setPointerCapture(event.pointerId); dragSurface.style.cursor = 'grabbing'; });
  dragSurface.addEventListener('pointermove', event => {
    if (penMode && penPoints.length && event.buttons) { const p = worldPoint(event), last = penPoints[penPoints.length - 1]; if (Math.hypot(p[0] - last[0], p[1] - last[1]) > 12 / view.scale) { penPoints.push(p); draw(); } return; }
  ...
  document.getElementById('sumoPenToggle')?.addEventListener('click', event => { penMode = !penMode; penPoints = []; event.currentTarget.classList.toggle('active', penMode); dragSurface.style.cursor = penMode ? 'crosshair' : 'grab'; const s = document.getElementById('sumoPenStatus'); if (s) s.textContent = penMode ? 'Pluma activa: haz clic por vértices o arrastra; doble clic para cerrar.' : 'Modo mover capa activo.'; });
  document.getElementById('sumoPenUndo')?.addEventListener('click', () => { penPoints.pop(); draw(); });
  document.getElementById('sumoPenClear')?.addEventListener('click', () => { drawnRings = []; penPoints = []; localStorage.removeItem('sumoModule8DrawnWetlands'); renderPenOutput(); draw(); });
  document.getElementById('sumoPenCopy')?.addEventListener('click', async () => { renderPenOutput(); const text = document.getElementById('sumoPenOutput')?.value || ''; try { await navigator.clipboard.writeText(text); } catch (_) {} const s = document.getElementById('sumoPenStatus'); if (s) s.textContent = 'Geometría copiada. Pégamela en el chat para actualizar la página.'; });
  renderPenOutput();
