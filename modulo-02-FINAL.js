...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Esperar un poco para que D3 esté listo
  setTimeout(() => {
...
function selectNode(nodeId) {
  ...
  // Contar conexiones del nodo seleccionado
  ...
  // REDIBUJAR CON SOLO LAS CONEXIONES DEL NODO SELECCIONADO
  drawNetwork();
}
...
// DRAW NETWORK - VERSIÓN CORREGIDA
function drawNetwork() {
  ...
  // CREAR COPIAS DE LINKS CON REFERENCIAS A OBJETOS (no strings)
  const linksForSimulation = activeLinks.map(link => ({
    source: activeNodes.find(n => n.id === link.source),
    target: activeNodes.find(n => n.id === link.target),
