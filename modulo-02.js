// DATOS REALES DEL POT - PROCESADOS
const potData = {
  EEP: {
...
function buildNodeListUI() {
  ...
  nodeList.innerHTML = allNodes.map(node => `
...
function selectNode(nodeId) {
  ...
  const incomingLinks = allLinks.filter(l => l.target === nodeId && l.active).length;
  const outgoingLinks = allLinks.filter(l => l.source === nodeId && l.active).length;
  ...
  info.innerHTML = `
    <div class="node-info-content">
      ...
      <div class="node-info-connections">
        <div class="connection-count">Conexiones activas: ${incomingLinks + outgoingLinks}</div>
        <div class="connection-count">Estructura: ${selectedNode.structureName}</div>
      </div>
    </div>
  `;
...
function applyAction(action) {
  ...
  switch(action) {
    ...
    case 'disminuir':
      ...
      break;
    case 'conectar':
      alert('Selecciona el nodo con el que deseas conectar');
      break;
    case 'mover':
      const structures = Object.keys(potData);
      const newStructure = structures[Math.floor(Math.random() * structures.length)];
      selectedNode.structure = newStructure;
      selectedNode.color = potData[newStructure].color;
      selectedNode.structureName = potData[newStructure].name;
      break;
  }
...
function resetSimulation() {
  ...
  initializeNetwork();
  updateStats();
  document.getElementById('selectedNodeInfo').innerHTML = '<div class="info-placeholder">Selecciona un nodo para ver detalles</div>';
}
...
function updateNetwork() {
  ...
  networkSimulation = d3.forceSimulation(activeNodes)
    .force('link', d3.forceLink(activeLinks.map(d => ({
      source: d.source,
      target: d.target
    }))).distance(100))
  ...
  const link = svg.append('g')
    ...
    .attr('stroke', d => {
      ...
      if (d.strength === 'medium') return 'rgba(91,141,239,0.4)';
      return 'rgba(162,118,242,0.2)';
    })
    .attr('stroke-width', d => {
  ...
  const node = svg.append('g')
    ...
    .enter()
    .append('circle')
    .attr('r', d => (d.id === selectedNode?.id ? 12 : 8) * (d.influence || 1))
    .attr('fill', d => d.color)
    .attr('opacity', d => d.id === selectedNode?.id ? 1 : 0.7)
    .attr('stroke', d => d.id === selectedNode?.id ? '#fff' : 'rgba(255,255,255,0.2)')
    .attr('stroke-width', d => d.id === selectedNode?.id ? 2 : 1.5)
...
function updateStats() {
  ...
  const density = (activeLinks / (activeNodes * (activeNodes - 1) / 2)).toFixed(2);
  
  document.getElementById('totalNodes').textContent = activeNodes;
  document.getElementById('totalLinks').textContent = activeLinks;
  document.getElementById('density').textContent = density;
  
  document.getElementById('beforeNodes').textContent = activeNodes;
  document.getElementById('beforeLinks').textContent = activeLinks;
  document.getElementById('beforeDensity').textContent = density;
}
...
function updateComparison() {
  ...
  const afterDensity = (afterLinks / (afterNodes * (afterNodes - 1) / 2)).toFixed(2);
  
  const beforeNodes = 87;
  const beforeLinks = 156;
  
  const deltaNodos = afterNodes - beforeNodes;
  const deltaImpacto = ((afterLinks - beforeLinks) / beforeLinks * 100).toFixed(1);
  
  document.getElementById('afterNodes').textContent = afterNodes;
  document.getElementById('afterLinks').textContent = afterLinks;
  document.getElementById('afterDensity').textContent = afterDensity;
  
  document.getElementById('deltaNodos').textContent = deltaNodos > 0 ? '+' + deltaNodos : deltaNodos;
  document.getElementById('deltaImpacto').textContent = deltaImpacto > 0 ? '+' + deltaImpacto + '%' : deltaImpacto + '%';
}
