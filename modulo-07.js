(() => {
  ...
  start(() => {
    ...
    const partOneRows = [
      { name: "Dinámica hídrica", color: "#4bb9ef", components: "Espejo de agua, suelo húmedo, rondas, escorrentías, entradas y salidas de agua y Canal Los Ángeles.", process: "El agua circula, se acumula, disminuye o se desborda según lluvia, pendiente, suelo, obras y condiciones ambientales.", partsPurpose: "No. El agua, el suelo y el canal no eligen entre alternativas ni persiguen objetivos propios.", totalPurpose: "No. La totalidad no decide un objetivo propio; su comportamiento depende de condiciones físicas y ambientales.", category: "Determinista.", justification: "Sus partes y su totalidad no formulan propósitos ni eligen entre alternativas." },
    ...
    const renderSubsystemNetwork = (item) => { const graph = subsystemNetworks[item.name]; if (!graph) return ""; const byId = Object.fromEntries(graph.nodes.map((node) => [node.id, node])); return `<section class="process-network" style="--bubble-color:${item.color}"><strong><i class="fa-solid fa-diagram-project"></i> RED INTERNA · RELACIONES</strong><small>${graph.note}</small><div class="process-network-canvas"><svg viewBox="0 0 100 100" aria-hidden="true">${graph.edges.map((edge) => { const a = byId[edge.a]; const b = byId[edge.b]; return `<line x1="${a.x}" y1="${a.y}" x2="${b.x}" y2="${b.y}"><title>${edge.label}</title></line>`; }).join("")}</svg>${graph.nodes.map((node) => `<span class="process-network-node" style="left:${node.x}%;top:${node.y}%" title="${node.label}"><i class="fa-solid fa-circle-dot"></i><em>${node.label}</em></span>`).join("")}</div><small class="process-network-legend">Las líneas muestran procesos de relación entre unidades del mismo nivel.</small></section>`; };
