(() => {
  ...
  start(() => {
    ...
    const partOneRows = [
      { name: "Subsistema hídrico", color: "#4bb9ef", components: "Humedal, espejo de agua, ronda, suelo húmedo y Canal Los Ángeles.", process: "El agua circula, se acumula, disminuye o se desborda según el entorno.", category: "Determinista / mecánico integrado a lo ecológico.", justification: "Sus procesos físicos tienen regularidades observables, pero se relacionan con el resto del sistema ecológico." },
      { name: "Subsistema biótico", color: "#68d391", components: "Vegetación, fauna y hábitat; aves, insectos y arañas como información secundaria.", process: "La vegetación se recupera o invade; la fauna llega, se desplaza y cambia su presencia.", category: "Ecológico como conjunto; animado en la fauna y determinista en la vegetación.", justification: "Integra organismos y relaciones de hábitat sin convertir cada componente en un nodo equivalente." },
      { name: "Infraestructura y borde urbano", color: "#a6adb7", components: "Avenida Ciudad de Cali, edificios, cerramientos, senderos y áreas construidas.", process: "El borde se densifica, fragmenta, cierra o se transforma mediante decisiones urbanas.", category: "Determinista / mecánico como estructura física.", justification: "La infraestructura tiene forma material, aunque su transformación depende de decisiones sociales." },
      { name: "Movilidad y accesibilidad", color: "#f1cf5b", components: "Ciclorruta, recorridos peatonales, accesos, tráfico y Biblioteca El Tintal.", process: "Cambian recorridos, intensidad del tránsito, ruido, accesibilidad y presión sobre el borde.", category: "Compuesto: mecánico, animado y social.", justification: "Combina infraestructura, usuarios individuales y organizaciones que regulan o habilitan desplazamientos." },
      { name: "Social-comunitario", color: "#ee9a4b", components: "Barrios, recorridos, actividades pedagógicas, visitas, juntas y organizaciones.", process: "Las prácticas aumentan, disminuyen o se reorganizan según acceso, conflicto y participación.", category: "Social; las personas individuales son sistemas animados.", justification: "Las comunidades y organizaciones actúan con propósitos colectivos y capacidad de coordinación." },
      { name: "Institucional y de manejo", color: "#b28be8", components: "Restauración, mantenimiento, control de invasoras, compostaje, seguimiento y educación ambiental.", process: "Cambian programas, responsabilidades, coordinaciones, prioridades y decisiones de manejo.", category: "Social.", justification: "Las entidades tienen reglas, propósitos, responsabilidades y capacidad de decisión." }
    ];
    ...
    const partOneLayerMeta = [
      { id: "hidrico", label: "Subsistema hídrico", color: "#4bb9ef", description: "Humedal, espejo de agua, ronda, suelo húmedo y Canal Los Ángeles." },
    ...
    const initPartOneControls = (map, layers) => {
      ...
      if (controls) {
        ...
        controls.querySelectorAll("[data-layer-toggle]").forEach((input) => input.addEventListener("change", () => {
          ...
          [layer.fill, layer.line, layer.point].forEach((id) => { if (map.getLayer(id)) map.setLayoutProperty(id, "visibility", input.checked ? "visible" : "none"); });
    ...
    function initRealCartography() {
      ...
      map.on("load", () => { const burro = { type: "FeatureCollection", features: [{ type: "Feature", properties: { name: "Humedal El Burro" }, geometry: { type: "Polygon", coordinates: [[[-74.166,4.644],[-74.159,4.647],[-74.153,4.644],[-74.151,4.638],[-74.156,4.633],[-74.163,4.634],[-74.168,4.639],[-74.166,4.644]]] } }] }; const context = { type: "FeatureCollection", features: [ { type: "Feature", properties: { name: "Avenida Ciudad de Cali", kind: "avenue" }, geometry: { type: "LineString", coordinates: [[-74.159,4.67],[-74.159,4.65],[-74.159,4.63],[-74.159,4.61]] } }, { type: "Feature", properties: { name: "Canal Los Ángeles", kind: "canal" }, geometry: { type: "LineString", coordinates: [[-74.181,4.649],[-74.174,4.645],[-74.167,4.641],[-74.158,4.637]] } }, { type: "Feature", properties: { name: "Ciclorruta", kind: "cycle" }, geometry: { type: "LineString", coordinates: [[-74.15,4.632],[-74.156,4.631],[-74.164,4.632],[-74.172,4.636]] } }, { type: "Feature", properties: { name: "Biblioteca El Tintal", kind: "library" }, geometry: { type: "Point", coordinates: [-74.156,4.633] } }, { type: "Feature", properties: { name: "Río Tunjuelo · cuenca", kind: "river" }, geometry: { type: "LineString", coordinates: [[-74.205,4.59],[-74.19,4.60],[-74.175,4.61],[-74.16,4.62]] } } ] }; map.addSource("burro", { type: "geojson", data: burro }); map.addSource("burro-context", { type: "geojson", data: context });
        ...
        partOneLayerMeta.forEach((meta) => {
          ...
          map.addLayer({ id: pointId, type: "circle", source: sourceId, filter: ["==", ["geometry-type"], "Point"], paint: { "circle-color": meta.color, "circle-radius": ["interpolate", ["linear"], ["zoom"], 10, 4, 14, 7], "circle-stroke-color": "#070b0c", "circle-stroke-width": 1.5 }, layout: { visibility: "none" } });
          partOneMapLayers.push({ id: meta.id, fill: fillId, line: lineId, point: pointId });
        });
    ...
    const subsystemData = [
      { name: "Dinámica del agua", short: "DINÁMICA\nDEL AGUA", color: "#4bd7ef", x: 12, y: 17, category: "Espejo de agua, suelo húmedo, rondas, escorrentías, entradas y salidas, Canal Los Ángeles.", process: "El agua circula, se acumula, disminuye, se desborda o transporta residuos según las condiciones del entorno.", actants: ["espejo de agua", "suelo húmedo", "rondas", "escorrentías", "Canal Los Ángeles"] },
      { name: "Suelo y soporte físico", short: "SUELO Y\nSOPORTE FÍSICO", color: "#c59b72", x: 32, y: 11, category: "Suelo, sedimentos, rellenos, escombros, zonas de manejo y protección ambiental.", process: "El suelo se compacta, se rellena, recibe residuos, retiene agua o cambia por obras y restauración.", actants: ["sedimentos", "rellenos", "escombros", "zonas de manejo"] },
      { name: "Vegetación", short: "VEGETACIÓN", color: "#67e8a5", x: 53, y: 11, category: "Buchón, junco, enea, ciro, aliso, espino, pasto kikuyo, acacias y eucalipto.", process: "Se recupera, se desplaza, se mantiene, se invade o se controla según las acciones de manejo.", actants: ["buchón", "junco", "enea", "ciro", "aliso", "pasto kikuyo"] },
      { name: "Fauna", short: "FAUNA", color: "#f5d76e", x: 75, y: 15, category: "Aves residentes, aves migratorias, insectos, arañas y otros organismos.", process: "Llegan, se desplazan, se reproducen, encuentran obstáculos o cambian su presencia según el hábitat.", actants: ["aves residentes", "aves migratorias", "insectos", "arañas"] },
      { name: "Hábitat y relaciones ecológicas", short: "HÁBITAT Y\nRELACIONES", color: "#9b8cff", x: 90, y: 30, category: "Agua, suelo, vegetación, fauna, alimento, refugio y condiciones del borde.", process: "El hábitat se transforma y reorganiza; cambia la capacidad del sector para sostener especies.", actants: ["alimento", "refugio", "borde", "relaciones ecológicas"] },
      { name: "Borde y fragmentación urbana", short: "BORDE Y\nFRAGMENTACIÓN", color: "#f5a623", x: 89, y: 50, category: "Avenida Ciudad de Cali, edificios, barrios, cerramientos y límites del humedal.", process: "El borde se densifica, se fragmenta, se cierra, se abre o cambia su relación con el ecosistema.", actants: ["Avenida Ciudad de Cali", "edificios", "barrios", "cerramientos"] },
      { name: "Movilidad y accesibilidad", short: "MOVILIDAD Y\nACCESIBILIDAD", color: "#f76fb0", x: 86, y: 70, category: "Tráfico, avenida, ciclorruta, recorridos peatonales, accesos y Biblioteca El Tintal.", process: "Cambian la intensidad del tránsito, los recorridos, el ruido, la accesibilidad y la presión sobre el borde.", actants: ["tráfico", "ciclorruta", "peatones", "Biblioteca El Tintal"] },
      { name: "Usos y prácticas comunitarias", short: "USOS Y PRÁCTICAS\nCOMUNITARIAS", color: "#b08cff", x: 66, y: 87, category: "Visitas, recorridos, educación ambiental, actividades pedagógicas, juntas y organizaciones.", process: "Aumentan, disminuyen o se reorganizan según accesos, actividades, conflictos y participación.", actants: ["visitas", "educación ambiental", "juntas", "organizaciones"] },
      { name: "Restauración ecológica", short: "RESTAURACIÓN\nECOLÓGICA", color: "#e89a6c", x: 44, y: 88, category: "Reintroducción de nativas, control de invasoras, mantenimiento, compostaje y seguimiento.", process: "Se interviene, se observa la respuesta del ecosistema y se ajustan las acciones.", actants: ["nativas", "invasoras", "mantenimiento", "compostaje"] },
      { name: "Gestión institucional", short: "GESTIÓN\nINSTITUCIONAL", color: "#ef8f8f", x: 22, y: 82, category: "Jardín Botánico, Secretaría de Ambiente, UAESP, Ciudad Limpia, Hospital del Sur y otras entidades.", process: "Cambian los programas, responsabilidades, coordinaciones, prioridades y decisiones de manejo.", actants: ["Jardín Botánico", "Secretaría de Ambiente", "UAESP", "Ciudad Limpia"] },
      { name: "Residuos y presiones urbanas", short: "RESIDUOS Y\nPRESIONES", color: "#e65d70", x: 10, y: 63, category: "Residuos del Canal Los Ángeles, escombros, residuos sólidos, ruido y emisiones del tráfico.", process: "Las presiones aparecen, se acumulan, se retiran o se transforman por acciones de manejo.", actants: ["residuos sólidos", "escombros", "ruido", "emisiones"] },
      { name: "Transformación histórica del territorio", short: "TRANSFORMACIÓN\nHISTÓRICA", color: "#d9a441", x: 10, y: 40, category: "Laguna El Tintal, reducción del área, urbanización y fragmentación actual.", process: "El sistema pasa de una extensión mayor a un humedal reducido y dividido.", actants: ["Laguna El Tintal", "reducción del área", "urbanización", "fragmentación"] },
    ];
    const drawSubsystems = ({ hidden = false } = {}) => {
      ...
      subsystemBubbles.querySelectorAll(".subsystem-bubble").forEach((button) => button.addEventListener("click", () => {
        ...
        subsystemBubbles.querySelectorAll(".subsystem-bubble, .subsystem-actants").forEach((node) => node.classList.remove("active"));
        button.classList.add("active");
        const children = document.createElement("div"); children.className = "subsystem-actants active"; children.style.setProperty("--bubble-color", item.color); children.style.left = `clamp(10px, ${Math.min(item.x + 5, 58)}%, calc(100% - 270px))`; children.style.top = `clamp(58px, ${Math.min(item.y + 8, 68)}%, calc(100% - 150px))`; children.innerHTML = `<strong>${item.name}</strong><span>${item.process}</span><small>${item.category}</small><div>${item.actants.map((actant) => `<i>${actant}</i>`).join("")}</div>`; subsystemBubbles.appendChild(children);
      }));
    };
