(() => {
  ...
  start(() => {
    ...
    function initRealCartography() {
      ...
      const map = new maplibregl.Map({ container: el, center: [-74.158, 4.638], zoom: 13.15, minZoom: 11, maxZoom: 17, attributionControl: false, style: { version: 8, glyphs: "https://demotiles.maplibre.org/font/{fontstack}/{range}.pbf", sources: { "local-bogota": { type: "image", url: "assets/bogota-osm-detail-gray.jpg", coordinates: [[-74.25,4.82],[-73.95,4.82],[-73.95,4.50],[-74.25,4.50]] }, dark: { type: "raster", tiles: ["https://a.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}@2x.png", "https://b.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}@2x.png", "https://c.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}@2x.png"], tileSize: 256, attribution: "© OpenStreetMap © CARTO" } }, layers: [ { id: "local-bogota-base", type: "raster", source: "local-bogota", paint: { "raster-opacity": .98, "raster-saturation": -1, "raster-contrast": .42, "raster-brightness-min": .1, "raster-brightness-max": .92 } }, { id: "dark-base", type: "raster", source: "dark", paint: { "raster-opacity": 0, "raster-saturation": -.45, "raster-contrast": .2 } } ] } });
    ...
    const subsystemData = [
      { name: "Dinámica hídrica", short: "DINÁMICA\nHÍDRICA", color: "#4bd7ef", x: 17, y: 26, category: "Ecológico, con componentes físicos deterministas.", process: "Entrada, circulación, acumulación y salida del agua; relación con el Canal Los Ángeles; transporte de residuos y condiciones de la ronda.", actants: ["Canal Los Ángeles", "escorrentía", "residuos", "ronda hídrica"] },
      { name: "Transformación ecológica", short: "TRANSFORMACIÓN\nECOLÓGICA", color: "#67e8a5", x: 42, y: 18, category: "Ecológico y animado.", process: "Cambios del espejo de agua, vegetación nativa e invasora, presencia de insectos, aves residentes y aves migratorias.", actants: ["aves migratorias", "vegetación nativa", "insectos", "especies invasoras"] },
      { name: "Fragmentación y transformación urbana", short: "FRAGMENTACIÓN\nURBANA", color: "#f5a623", x: 76, y: 25, category: "Determinista o mecánico en la infraestructura, relacionado con sistemas sociales.", process: "Reducción histórica del área, división por la Avenida Ciudad de Cali, crecimiento de edificaciones y modificación de los bordes.", actants: ["Avenida Ciudad de Cali", "edificaciones", "escombros", "bordes urbanos"] },
      { name: "Movilidad y accesibilidad cotidiana", short: "MOVILIDAD Y\nACCESIBILIDAD", color: "#f76fb0", x: 78, y: 54, category: "Mecánico, animado y social.", process: "Tráfico, ruido, contaminación, recorridos por la ciclorruta, accesos y relación con la Biblioteca El Tintal.", actants: ["ciclorruta", "Biblioteca El Tintal", "peatones", "tráfico y ruido"] },
      { name: "Usos y prácticas comunitarias", short: "USOS Y\nPRÁCTICAS", color: "#b08cff", x: 60, y: 80, category: "Animado y social.", process: "Visitas, educación ambiental, participación, apropiación, recorridos y acciones de organizaciones y juntas.", actants: ["Juntas de Acción Comunal", "organizaciones", "visitantes", "educación ambiental"] },
      { name: "Restauración y manejo adaptativo", short: "RESTAURACIÓN\nADAPTATIVA", color: "#e89a6c", x: 30, y: 80, category: "Social, relacionado con lo ecológico.", process: "Reintroducción de vegetación, control de invasoras, mantenimiento, compostaje, educación y coordinación institucional.", actants: ["Jardín Botánico", "mantenimiento", "compostaje", "instituciones"] },
    ];
    const drawSubsystems = () => {
      if (!subsystemBubbles) return;
      subsystemBubbles.innerHTML = subsystemData.map((item, index) => `<button class="subsystem-bubble" data-subsystem="${index}" style="--bubble-color:${item.color};left:${item.x}%;top:${item.y}%" title="${item.name}">${item.short}</button>`).join("");
