(() => {
  ...
  start(() => {
    ...
    function initRealCartography() {
      ...
      map.once("load", () => { const roadLayers = (map.getStyle().layers || []).filter((layer) => layer.type === "line" && /road|street|highway|transportation|motorway|trunk|primary|secondary|tertiary|residential/i.test(layer.id)); roadLayers.forEach((layer) => { try { map.setPaintProperty(layer.id, "line-width", ["interpolate", ["linear"], ["zoom"], 10, .38, 13, .72, 16, 1.35]); map.setPaintProperty(layer.id, "line-opacity", .72); } catch (error) { /* algunas capas de etiquetas no aceptan estas propiedades */ } }); window.setTimeout(loadOsm, 450); });
